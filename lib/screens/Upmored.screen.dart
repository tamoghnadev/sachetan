...
import '../services/geotagservice.dart';

class UpmoredScreen extends StatefulWidget {

  var Idd;
  var ltt;
  var lnn;

  UpmoredScreen({Key? key,required this.Idd,required this.ltt,required this.lnn}) : super(key: key);

  @override
  _UpmoredScreenState createState() => _UpmoredScreenState();
}

class _UpmoredScreenState extends State<UpmoredScreen> {

  var progress;
  var selectedfile;
  late Response response;
  var _geoservice = GeoTagService();
  String pddf = "";
  String result = "";
  String img = "";
  Dio dio = new Dio();
  ...
  void initState() {
    ...
    //print("Kalla"++widget.+);
    uploadGeoLoc(widget.Idd.toString(),widget.ltt.toString(),widget.lnn.toString());
    

    super.initState();
  }

  uploadGeoLoc(String id,String latt,String Langg) async {
    try {
     var papa = GeoTag();
      papa.cpid = id;
      papa.latitude = latt;
      papa.longitude = Langg;
      final _result = await _geoservice.updategeoticket(papa);
      print(_result.body.toString());
      if (_result.body != null) {
        final data = json.decode(_result.body);
        if (data['result']) {
          Fluttertoast.showToast(
              msg: "Location Has Been Tagged Successfully.",  // message
          ...
          /*Timer(Duration(seconds: 1), () {
            ...
            Navigator.push(context,
                MaterialPageRoute(builder: (context) => UploaddocScreen(Idd:widget.Idd)));
          });*/
        }
      }
    } catch (e) {
