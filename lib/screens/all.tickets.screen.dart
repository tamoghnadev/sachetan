...
class _AllTicketsScreenState extends State<AllTicketsScreen> {
  ...
  _getAllTickets() async {
    try {
      ...
      var result = await _ticketService.getTicketsByUserId(
      ...
      tickets.forEach((data) {
        ...
        if (data['is_opened'] == 1 || data['is_opened'] == '1') {
          ...
          setState(() {
            status = Text(
              '${getTranslated(context, 'Resolved')}',
          ...
          setState(() {
            status = Text(
              '${getTranslated(context, 'UnResolved')}',
  ...
  Future _closeATicket(int? ticketId) async {
    try {
      return await _ticketService.closeATicket(
          ticketId!);
    } catch (e) {
      return e.toString();
    }
                      ...
                      Padding(
                        ...
                        child: ButtonTheme(
                          ...
                          child: TextButton(
                            onPressed: () async {
                              Navigator.pop(context);
                              _confirm(context, ticket, '${getTranslated(context, 'Close')}',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('${getTranslated(context, 'All Tickets')}'),
        actions: <Widget>[
          _isCustomer == true
              ? InkWell(
                  onTap: () {
